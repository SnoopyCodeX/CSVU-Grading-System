        ...
        <div class="px-4 flex justify-between flex-col gap-4 mt-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center">
                ...
                <div class="flex flex-col md:flex-row md:items-center gap-4 px-4 w-full md:w-auto">
                    ...
                    <form x-data="{placeholder: 'Search subject'}" method="POST"
                        class="md:grid md:grid-cols-1 gap-4 hidden md:block" autocomplete="off">
                        <div class="join w-[280px] md:w-auto mb-2 md:mb-0">
                            <div>
                                <div>
                                    <input class="input input-bordered join-item" name="search-subject"
                                        :placeholder="placeholder"
                                        value="<?= (isset($search) ? ($searchBy == 'course' ? $course : $search) : '') ?>"
                                        required />
                                </div>
                            </div>

                            <select class="select select-bordered join-item hidden md:block" name="search-by"
                                @change="placeholder = 'Search ' + $event.target.value.replace(/_/gi, ' ').replace(/term/gi, 'semester').replace(/name/gi, 'subject')"
                                required>
                                <option disabled>Search by</option>
                                <option value="name" <?php if (isset($search) && strtolower($searchBy) == 'name'): ?>
                                    selected <?php endif; ?>>Subject</option>
                                <option value="course"
                                    <?php if (isset($search) && strtolower($searchBy) == 'course'): ?> selected
                                    <?php endif; ?>>Course</option>
                                <option value="year_level"
                                    <?php if (isset($search) && strtolower($searchBy) == 'year_level'): ?> selected
                                    <?php endif; ?>>Year Level
                                </option>
                                <option value="term" <?php if (isset($search) && strtolower($searchBy) == 'term'): ?>
                                    selected <?php endif; ?>>Semester</option>
                            </select>
                    ...
                    <form x-data="{placeholder: 'Search subject'}" method="POST"
                        class="grid grid-cols-1 gap-2 block md:hidden !w-full" autocomplete="off">
                        <div class="join w-full md:w-auto mb-2 md:mb-0">
                            <input class="input input-bordered w-full join-item" name="search-subject"
                                :placeholder="placeholder"
                                value="<?= (isset($search) ? ($searchBy == 'course' ? $course : $search) : '') ?>"
                                required />
                        </div>
                        ...
                        <div class="flex gap-2 w-full block md:hidden">
                            <select class="select select-bordered block md:hidden w-full" name="search-by"
                                @change="placeholder = 'Search ' + $event.target.value.replace(/_/gi, ' ').replace(/term/gi, 'semester').replace(/name/gi, 'subject')"
                                required>
                                <option disabled>Search by</option>
                                <option value="name" <?php if (isset($search) && strtolower($searchBy) == 'name'): ?>
                    ...
                    <!-- Import Button -->
                    <button class="btn text-white bg-[#276bae]" onclick="import_file_modal.showModal()"><i
                            class="bx bx-import"></i>
                        Import</button>

                    <!-- Create button -->
                    <a href="./create/subject.php" class="btn text-white bg-[#276bae]"><i class="bx bx-plus-circle"></i>
                        Create</a>
                </div>
            </div>
                        ...
                        <tr class="hover">
                            ...
                            <td>
                                <div class="flex justify-center gap-2">
                                    <a href="./view/subject_instructors.php?subject=<?= $subject['id'] ?><?= $page != 1 ? '&prev_page=' . $page : '' ?>"
                                        class="btn bg-[#276bae] btn-sm text-white">
                                    ...
                                    <Edit for="edit-subject-<?= $subject['id'] ?>"
                                    ...
                                    <label for="delete-subject-<?= $subject['id'] ?>"
            ...
            <form class="flex flex-col gap-4 px-[32px] mb-auto" method="post">
                ...
                <label class="flex flex-col gap-2">
                    ...
                    <select class="select select-bordered" name="course" required>
                        ...
                        <option value="<?php echo $course['id'] ?>" <?php if ($subject['course'] == $course['id']) { ?>
                            selected <?php } ?>><?php echo $course['course'] . " - #" . $course['course_code'] ?>
                        </option>
                        <?php } ?>
                    </select>
                </label>
                ...
                <label class="flex flex-col gap-2">
                    ...
                    <select class="select select-bordered" name="year_level" required>
                        ...
                        <option value="1st year" <?php if ($subject['year_level'] == "1st year") { ?> selected
                            <?php } ?>>
                            1st year</option>
                        <option value="2nd year" <?php if ($subject['year_level'] == "2nd year") { ?> selected
                            <?php } ?>>
                            2nd year</option>
                        <option value="3rd year" <?php if ($subject['year_level'] == "3rd year") { ?> selected
                            <?php } ?>>
                            3rd year</option>
                        <option value="4th year" <?php if ($subject['year_level'] == "4th year") { ?> selected
                            <?php } ?>>
                            4th year</option>
                    </select>
                </label>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <label class="flex flex-col gap-2">
                        ...
                        <input class="input input-bordered" placeholder="Enter Subject Name" name="subject_name"
                            value="<?= $subject['name'] ?>" required />
                    </label>

                    <label class="flex flex-col gap-2">
                        <span class="font-bold text-[18px]">Subject Code</span>
                        <input class="input input-bordered" placeholder="Enter Subject Code" name="subject_code"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="flex flex-col gap-2">
                        ...
                        <input class="input input-bordered" placeholder="Enter Subject Units" name="units"
                            value="<?= $subject['units'] ?>" required />
                    </label>

                    <label class="flex flex-col gap-2">
                        <span class="font-bold text-[18px]">Credits Units</span>
                        <input class="input input-bordered" placeholder="Enter Subject Credits" name="credits_units"
                            value="<?= $subject['credits_units'] ?>" required />
                    </label>
        ...
        <div class="modal-box min-w-[474px]">
            ...
            <form class="flex flex-col gap-4" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-action">
                    <button class="btn btn-sm md:btn-md text-black bg-gray-400 hover:text-white hover:bg-[#276bae] "
                        type="button" onclick="downloadTemplate()"><i class="fa fa-download"></i> Download
                        template</button>
                    <button class="btn btn-sm md:btn-md text-black bg-gray-400 hover:text-white hover:bg-red-500"
                        type="button" onclick="import_file_modal.close()">Cancel</button>
