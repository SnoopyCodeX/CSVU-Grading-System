...
<div class=" navbar bg-base-100 flex justify-between items-center my-4">
    ...
    <div class="flex justify-between items-center w-full">
        <div class="flex flex-col gap-1 items-start">
            <h1 class="text-[1em] md:text-[1.6em] font-bold"><?= AuthController::user()->firstName ?>
                <?= AuthController::user()->middleName ?> <?= AuthController::user()->lastName ?>
            </h1>
            <h1 class="badge badge-info"><i class="bx bxs-user"></i> Instructor</h1>
        </div>
    ...
    <div class="modal-box">
        ...
        <form class="flex flex-col gap-4  px-[8px] mb-auto" action="" method="post">
            <label class="flex flex-col gap-2" x-data="{show: true}">
                ...
                <div class="relative">
                    <input class="input input-bordered w-full" name="old-password" placeholder="Old password"
                        x-bind:type="show ? 'password' : 'text'" required />
                    <button type="button"
                        class="btn btn-ghost absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5"
                        @click="show = !show">
            ...
            <label class="flex flex-col gap-2" x-data="{show: true}">
                ...
                <div class="relative">
                    <input class="input input-bordered w-full" name="new-password" placeholder="New password"
                        x-bind:type="show ? 'password' : 'text'" required />
                    <button type="button"
                        class="btn btn-ghost absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5"
                        @click="show = !show">
            ...
            <label class="flex flex-col gap-2" x-data="{show: true}">
                ...
                <div class="relative">
                    <input class="input input-bordered w-full" name="confirm-password" placeholder="Confirm password"
                        x-bind:type="show ? 'password' : 'text'" required />
                    <button type="button"
                        class="btn btn-ghost absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5"
                        @click="show = !show">
            ...
            <ul class="menu">
                ...
                <li>
                    <a href="<?php echo $rootFolder; ?>/manage-grading-criteria.php">
                        ...
                        <span class="text-[18px]"> Grading Criterias</span>
                    </a>
                </li>
                <li>
